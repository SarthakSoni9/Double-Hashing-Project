...
function getSelectedTableSize() {
  const rawValue = Number(elements.tableSizeSelect?.value);
  if (!Number.isInteger(rawValue)) {
    return DEFAULT_TABLE_SIZE;
  }

  return Math.min(MAX_TABLE_SIZE, Math.max(MIN_TABLE_SIZE, rawValue));
}
...
async function analyzeKey() {
  ...
  const selectedTableSize = getSelectedTableSize();
  if (selectedTableSize !== state.tableSize) {
...
function init() {
  state.tableSize = getSelectedTableSize();
  syncTableConfig();
  updateVisualizerLabels();
  renderGrid();
  renderMiniMap();
